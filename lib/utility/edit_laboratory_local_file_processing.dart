...
import 'package:diligov_members/models/signature.dart';
...
import 'package:diligov_members/utility/draggable_member_widget.dart';
...
import '../widgets/draggable_signature_widget.dart';

class EditLaboratoryLocalFileProcessing extends StatefulWidget {
  final Minute minute;
  const EditLaboratoryLocalFileProcessing({super.key, required this.minute});

  @override
  State<EditLaboratoryLocalFileProcessing> createState() => _EditLaboratoryLocalFileProcessingState();
}

class _EditLaboratoryLocalFileProcessingState extends State<EditLaboratoryLocalFileProcessing> {
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: IconButton(
                      ...
                      onPressed: () {
                          provider.textAnnotations.removeWhere((a) => a.id == newId); // Delete annotation
                          int indexToRemove = provider.textList.indexWhere((item) => item['newId'] == newId);
                          if (indexToRemove != -1) {
                ...
                if (!isNew)
                  TextButton(
                    onPressed: () {
                        ...
                        int indexToRemove = provider.textList.indexWhere((item) => item['newId'] == annotation.id);
                        if (indexToRemove != -1) {
                          provider.textList.removeAt(indexToRemove);
                        }
              ...
              return AlertDialog(
                ...
                actions: <Widget>[
                  TextButton(
                    onPressed: () {
                        print(annotation.id);
                        int indexToRemove = provider.textList.indexWhere((item) => item['oldId'] == annotation.id);
                        if (indexToRemove != -1) {
                          provider.textList.removeAt(indexToRemove);
                        }
  ...
  Future<void> preparePdfFileFromNetwork() async {
    ...
    try {
      if(await PDFApi.requestPermission()){
        print('https https ------------------- https://diligov_members.com/public/meetings/'+widget.minute.minuteFile!);
  ...
  Future<void> takeScreenshot() async {
    ...
    try {
      if(await PDFApi.requestPermission()){
        ...
        List<Map<String, dynamic>> recordsFiles = await laboratoryProvider.processAndCollectFileInfo();

        List<Map<String, dynamic>> canvasItemData = laboratoryProvider.canvasItemsList.map((canvasItem) => canvasItem.toMap()).toList();
        List<Map<String, dynamic>> signaturesData = laboratoryProvider.signatures.map((sig) => sig.toMap()).toList();
        widget.minute.notes?.map((e) =>
            laboratoryProvider.textList.add({
              "oldId": e.id,
              "text": e.text,
        ...
        Future.delayed(Duration.zero, () {
          provider.updateMinuteNote(data);
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      userProvider.getListOfMemberMenu();
    });
        ...
        builder: (BuildContext context, laboratoryFileProcessingProvider, child) {
          return Form(
            child: RepaintBoundary(
              ...
              child: laboratoryFileProcessingProvider.localPath.isNotEmpty ? GestureDetector(
                ...
                child: DragTarget<Member>(
                  onAcceptWithDetails: (details) {
                    ...
                    // Call the provider's method to add or update the signature
                    laboratoryFileProcessingProvider.addOrUpdateSignature(
                      memberId: details.data.memberId.toString(),
                      memberName: details.data.memberFirstName ?? 'Unknown',
                    ...
                    return SafeArea(
                      child: Stack(
                        children: [
                          ...
                          if (widget.minute.audioNotes!.isNotEmpty)
                            SizedBox(
                              width: 300,
                              height: 400,
                              child: ListView.builder(
                                itemCount: widget.minute.audioNotes!.length,
                                itemBuilder: (BuildContext context, int index) {
                                  return GestureDetector(
                                    child: Container(
                                      ...
                                      child: ListTile(
                                        title: CustomText(
                                          text: widget.minute.audioNotes![index].audioNoteName!,
                                          fontSize: 20,
                                          color: Colors.black,
                                        ),
                                        trailing:
                                        Consumer<AudioRecordingProvider>(
                                            builder:
                                              ...
                                              return IconButton(
                                                onPressed: () async {
                                                  final String dir = '${AppUri.baseUntilPublicDirectory}/record_notes/${widget.minute.audioNotes![index].businessId!.toString()}/notes/records/${widget.minute.audioNotes![index].audioNoteRandomName!}';
                                                  print(dir);
                          ...
                          if (widget.minute.notes!.isNotEmpty)
                            ...widget.minute.notes!
                                .where((annotation) => annotation.pageIndex == laboratoryFileProcessingProvider.indexing)
                          ...
                          if (widget.minute.canvasItems!.isNotEmpty)
                            ...widget.minute.canvasItems!.asMap().entries.where((stroke) => stroke.value.pageIndex ==  laboratoryFileProcessingProvider.indexing).map((entry) {
                          ...
                          // Display signatures for the current page
                          ...laboratoryFileProcessingProvider
                              .getSignaturesForCurrentPage(laboratoryFileProcessingProvider.indexing.toString())
                              .map((signature) {
                            return DraggableSignatureWidget(
                              signature: signature,
                              signatureIndex: laboratoryFileProcessingProvider.indexing,
                            );
                          }),

                          if (laboratoryFileProcessingProvider.showUsers)
                                    ...
                                    child: Container(
                                      ...
                                      child: ListView.builder(
                                        ...
                                        itemBuilder: (context, index) {
                                          return DraggableMemberWidget(member: provider.dataOfMembers!.members![index]);
                                        },
                                      ),
                                    ),
                                  ),
